...
\subsection{Problem}
    \begin{frame}[fragile]{Problem}
        \begin{itemize}
            ...
            \end{itemize}
            \item<3-> To avoid using large number arithmetic, in practice \textbf{stream variants} are used, which enforce by \textcolor{orange}{renormalization}\refer{4}
        \end{itemize}
    \end{frame}
...
\begin{frame}[fragile]{}
    \begin{itemize}
        ...
        \item<2-> Specifically, we will enforce $\mathit{x}$ to remain in some fixed range $\mathit{I}$  \textcolor{orange}{by transferring the least significant bits to the stream} \refer{3}
        \item<3-> For generality, let us transfer a base $\mathit{b}$ digit, for example $\mathit{b}= 2$ means transferring single bits, while $\mathit{b} = 2^8$ means transferring complete bytes \refer{3}
        \item<4-> In rANS: 
        \begin{itemize}
...
\end{frame}
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{maxX[s]}$? \refer{5}}}
    \begin{center}
...
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{maxX[s]}$? \refer{5}}}
    ...
    \end{center}
    \begin{itemize}
        \item<1-> Assume that $\mathit{x_i = H_s +1}$ and it will be shrunk to $\mathit{x_i'=\lfloor x_i/b \rfloor}$
        \item<2-> $\mathit{x_i' = \lfloor x_i/b \rfloor = \lfloor (H_s + 1)/b \rfloor >= L_s}$
        \item<3-> $\mathit{ L_s <= \lfloor (H_s + 1)/b \rfloor <= (H_s + 1)/b}$
        \item<4-> $\mathit{H_s >= b \cdot L_s - 1}$, $\forall \mathit{s} \in \mathcal{A}$ \textcolor{blue}{(Constraint 1)}
    \end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{L,H}$ ? \refer{5}}}
    \begin{center}
...
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{L,H}$ ? \refer{5}}}
    \begin{itemize}
        \item<1-> $\mathit{x_i' = b \cdot x_{i} + \text{bit}}$
        \item<1-> Either $\mathit{x_i'}$ in $\mathit{[L, H]}$ or $\mathit{x_{i}}$ in $\mathit{[L, H]}$. Assume that $\mathit{x_{i}}$ in $\mathit{[L, H]}$ \refer{6}:
            \begin{itemize}
                \item<2-> $\mathit{x_{i} >= L}$
                \item<3-> $\mathit{x_i' >= b \cdot L + bit >= b \cdot L}$
                \item<4-> $\mathit{x_i'}$ can't be in $\mathit{[L, H]}$
                \item<5-> $\Longrightarrow \mathit{H <= b\cdot L -1}$
            \end{itemize}
        \item<6-> $\mathcal{I}=\mathit{[L,H]}$, where $\mathit{H <= b\cdot L -1}$ \textcolor{blue}{(Constraint 2)}
        \item<7-> $\mathcal{I}\mathit{=\{L, ..., b\cdot L -1\}}$, which is defined by Jarek Duda \textbf{$\mathit{b}$-unique interval} and $\mathit{L \in \mathbb{N}^+}$
        \item<8-> This will ensure that the loop in Method 1 will always return to the original value
    \end{itemize}
\end{frame}
