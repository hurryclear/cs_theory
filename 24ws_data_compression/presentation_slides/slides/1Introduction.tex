...
\subsection{Motivation}
    \begin{frame}[fragile]{Motivation}
        \begin{itemize}
            \item Huffman coding is efficient for symbol-based compression but cannot achieve optimal compression in many cases due to its restriction to \textbf{integer-length codes}
            \item Arithmetic coding achieves near-optimal compression but has a higher computational cost, especially in encoding and decoding ( it uses \textbf{multiplication and division operations for each encoded symbol})
            \item Asymmetric Numberal System introduced by Jarek Duda from Jagiellonian University in 2013\\ ANS combines the compression ratio of \href{https://en.wikipedia.org/wiki/Arithmetic_coding}{arithmetic coding} (integer arithmetic, modular arithmetic, and table lookups), with a processing cost similar to that of \href{https://en.wikipedia.org/wiki/Huffman_coding}{Huffman coding} \refer{4}
        \end{itemize}
    \end{frame}
