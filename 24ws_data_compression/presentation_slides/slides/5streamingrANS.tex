...
\begin{frame}[fragile]{Renormalization in Range Coding}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item 2
        \end{itemize}
        \item <2-> we use the similar idea for rANS
        \begin{itemize}
...
\begin{frame}[fragile]{Methods from Jarek Duda: Method 1 - Decoding \refer{2}}
    \pseudo{ANS decoding step from state $x$}{
        ...
        \quad $\mathit{\text{expand}(x,m) = m \cdot x + \text{readDigit}()} \rightarrow \mathit{x}$\\
        \textbf{end while} \\
...
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{L,H}$ ?}}
    \begin{itemize}
        \item $\mathit{x_i = m \cdot x_{i-1} + \text{bit}}$
        \item Either $\mathit{x_i}$ in $\mathit{[L, H]}$ or $\mathit{x_{i-1}}$ in $\mathit{[L, H]}$. Assume $\mathit{x_{i-1}}$ in $\mathit{[L, H]}$.
            \begin{itemize}
                ...
                \item $\mathit{x_i >= m \cdot L + bit >= m \cdot L}$
                \item $\mathit{x_i}$ can't be in $\mathit{[L, H]}$
                \item derive: $\mathit{H <= m\cdot L -1}$
            \end{itemize}
        \item $\mathit{[L,H]}$, where $\mathit{H <= m\cdot L -1}$ \textcolor{blue}{(Constraint 1)}
        \item $\mathit{I=\{L, ..., b\cdot L -1\}}$, which is defined by Jarek Duda $\mathit{b}$-unique interval and $\mathit{L \in \mathbb{N}}$
...
\begin{frame}[fragile]{Methods from Jarek Duda: Method 2 - Encoding \refer{2}}
    ...
    \pseudo{ANS encoding step of symbol sfrom state $\mathit{x}$}{
        ...
        \quad writeDigit(mod($\mathit{x,m}$)) \\
        \quad $\mathit{\text{shrink}(x,m)=\lfloor x/m \rfloor \rightarrow x}$ \textcolor{gray}{\fontsize{9}{12}\selectfont\textit{\# shrink the x}}\\
        \textbf{end while} \\
...
\begin{frame}[fragile]{\textcolor{red}{What is $\mathit{maxX[s]}$?}}
    \begin{itemize}
        ...
        \item Assume that $\mathit{x_i = H_s + 1}$ and it will be shrinked to $\mathit{x_i'=\lfloor x_i/m \rfloor}$
        \item $\mathit{x_i' = \lfloor x_i/m \rfloor = \lfloor (H_s + 1)/m \rfloor >= L_s}$
        \item $\mathit{ L_s <= (H_s + 1) / m <= \lfloor (H_s + 1)/m \rfloor < (H_s + 1)/b + 1}$
        \item $\mathit{H_s >= m \cdot L_s - 1}$, $\forall \mathit{s} \in \mathcal{A}$ \textcolor{blue}{(Constraint 2)}
    \end{itemize}
\end{frame}
\begin{frame}[fragile]{Final Form of $[L,H]$ in practice}
    \begin{itemize}
        \item Contraints $$\begin{gathered}\mathit{[L,H]}, \text{ where } \mathit{H <= m\cdot L -1} \\ \mathit{H_s >= m \cdot L_s - 1}, \forall \mathit{s} \in \mathcal{A}\end{gathered}$$
        \item In practice $$\begin{gathered}\mathcal{I}=\mathit{[l\cdot m, 2l\cdot m-1]} \\ \mathcal{I}_{\mathit{s}} \mathit{=[l\cdot \mathit{F_s}, 2l \cdot F_s -1] \text{   for   } \forall \mathit{s} \in \mathcal{A}}\end{gathered}$$
    \end{itemize}
\end{frame}
